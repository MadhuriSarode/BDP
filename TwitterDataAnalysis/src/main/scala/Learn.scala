 ...
 * This also gives commercial advantage, for investement.
 */
object Learn{
  ...
  //  val moviesList = Set("AladdinTrailer.txt","AngryBirds2Trailer.txt","Frozen2Trailer.txt","LionKingTrailer.txt","Minions2.txt","ToyStory4Trailer.txt")
    ...
    def hashTagTrend(): Unit =
    {
      ...
      counts.coalesce(1).saveAsTextFile("Result/HashTagCount.txt")

      //Print the results
      val trendingTopic = "TRENDING HASHTAG = "+ counts.first() + "\n" +"===============================\n"
      val topicsList = "The first 5 trending hashtags are\n"
     /* val fileObject = new PrintWriter(new FileOutputStream(new File("Conclusion/TrendingMovieHashTag.txt"),true))   //true For appending
      val printWriter = new PrintWriter(fileObject)
